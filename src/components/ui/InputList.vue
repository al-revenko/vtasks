<template>
  <form data-id="InputList" @submit.prevent="callbacks.onItemAdd">
    <label class="form-control w-full">
      <div class="label pt-0">
        <span v-if="props.title" class="mr-auto label-text">{{ props.title }}</span>
        <span v-if="props.itemsMaxcount" class="ml-auto label-text"
          >{{ listModel.length }}/{{ props.itemsMaxcount }}</span
        >
      </div>
      <div v-if="listModel.length < (props.itemsMaxcount ?? Infinity)" :class="`mb-2 flex gap-2`">
        <InputData
          type="text"
    ...
    <template v-if="listModel.length > 0">
      ...
      >
        ...
        >
          ...
          <CloseBtn type="button" :class="'btn-xs'" @click="() => callbacks.onItemDelete(i)" />
        </li>
      </ul>
    </template>
  </form>
